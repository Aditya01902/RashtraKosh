...
async function main() {
    console.log("LOG: Final Ingestion (Re-run with data) starting...");
    let count = 0;
    for (const item of DATA) {
        try {
            const m = await prisma.ministry.upsert({
                ...
                create: { 
                    name: item.ministry, 
                    shortCode: "S" + Math.random().toString(36).substring(2,7).toUpperCase(), 
                    color: "#3b82f6", 
                    sector: Sector.OTHER 
                }
            });
            ...
            await prisma.budgetAllocation.upsert({
                ...
                update: { 
                    allocated: item.be_26_27, 
                    utilized: item.actuals_24_25, 
                    revisedEstimate: item.re_25_26 
                },
                create: { 
                    schemeId: s.id, fiscalYear: fy, 
                    allocated: item.be_26_27, utilized: item.actuals_24_25, revisedEstimate: item.re_25_26,
            ...
            // Also create dummy for other years to avoid cleanup error
            const otherYears = ["2023-24", "2022-23"];
            for (const oy of otherYears) {
                await prisma.budgetAllocation.upsert({
                    where: { schemeId_fiscalYear: { schemeId: s.id, fiscalYear: oy } },
                    update: {},
                    create: { 
                        schemeId: s.id, fiscalYear: oy, 
                        allocated: 0, utilized: 0, revisedEstimate: 0,
                        allocatedCapital: 0, allocatedRevenue: 0, utilizedCapital: 0, utilizedRevenue: 0,
                        expenditureQ1: 0, expenditureQ2: 0, expenditureQ3: 0, expenditureQ4: 0, surrendered: 0
                    }
                });
    ...
    console.log(`LOG: Ingested ${count} schemes into DB.`);
}
